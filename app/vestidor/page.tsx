...
import Navbar from '@/components/Navbar' // Importamos tu nuevo Navbar unificado
...
export default function VestidorPage() {
  ...
  const handleConfirm = async () => {
    if (!proximoPartido || !proximoPartido.fecha) return alert('No hay fecha definida.');
    const fechaPartido = new Date(proximoPartido.fecha);
  ...
  if (!perfil.aprobado) return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center p-6 text-center">
      ...
      <button 
        ...
        className="mt-8 text-[10px] font-black uppercase tracking-[0.3em] text-zinc-700 hover:text-white transition-all cursor-pointer hover:scale-105 active:scale-95"
      >
        Cerrar Sesión
      </button>
    </div>
